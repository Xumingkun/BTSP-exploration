...
class DeepSoftHebb(nn.Module):
    ...
    def forward(self, x, is_training=False, task_id = None):
        ...
        if not is_training:
            ...
            acti_neuron = ((self.sparse_mask0[task_id] - 1) * self.sparse_mask[task_id]).view(1,self.projection_dim).to(device)  # -0.5
            # print('***', acti_neuron.shape)
            # print(acti_neuron)
            delta_w = torch.mean(out_flatten,dim=0).view(24576,1) @ acti_neuron * 0.1
